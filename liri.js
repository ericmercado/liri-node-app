...
function parseCommand(command, arg) {
    ...
    case undefined:
    case "":
        console.log("Hey boi, I might end my life!");
        break;
    default:
        console.log("DINKIE BO BEANIE TRY AGAIN IDIIOOTTT!");
        break;
    }
};
function myTweets(){
    var twitterUsername = process.argv[4];
        if(!twitterUsername){
            twitterUsername = "DRUMPFDINKSTER";
        }
...
function spotifySong(song) {
    song = (song || "Loose");
    console.log("That song? You're so PREDICTABLE.\n");
    var spotify = new Spotify(keys.spotify);
    ...
    .then(function(response) {
        ...
        for (var i = 0; i < response.tracks.items.length; i++) {
            if (response.tracks.items[i].name.toLowerCase() === song.toLowerCase()) {
                console.log("If it's not playing this blame the teachers for not teaching me correctly:\n");
        ...
        if (!foundSong) {
            console.log("Hey idiot,'" + song + "'doesn't exist on Spotify.");
        }
    })
    .catch(function(err) {
...
function movieInfo(movie) {
    ...
    request(queryUrl, function (error, response, body) {
        ...
        if (body && JSON.parse(body).Search && JSON.parse(body).Search.length > 0) {
            for (var i = 0; i < JSON.parse(body).Search.length; i++) {
                ...
                if (result.Title.toLowerCase() === movie.toLowerCase()) {
                    ...
                    request(innerQueryURL, function (error, response, body) { 
                        if (error) {
                            console.log("Another error? Man, you are REALLY FUCKING TERRIBLE AT THIS.\n  " + error);
                            return;
                        }
                        if (body && JSON.parse(body) && JSON.parse(body).Response === "True") {
                            body = JSON.parse(body);
                            console.log("ONLY LOSERS LIKE THIS SHEEEYYYYIIIITT:\n");
            ...
            console.log("Yeah'" + movie + "'isn't a real movie, maybe be SMARTER NEXT TIME.");
        }
    });
};
function parseFileCommand() {
    fs.readFile("random.txt", "utf8", function(error, data) {
        if (error) {
            return console.log("If you are alive, you are wrong.\n  " + error);
        }
